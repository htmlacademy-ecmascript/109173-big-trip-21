  ...
  return offers.map((offer) => {
    ...
    return `
      <div class="event__offer-selector">
        <input class="event__offer-checkbox  visually-hidden" id="event-offer-${loweredOfferName}-1" type="checkbox" name="event-offer-${loweredOfferName}">
...
function createEditPointTemplate({type, destination, dates, offers, cost}) {
  ...
  const dateStart = dayjs(dates.start).format(DATE_FORMATS.CHOSED_DATE);
  const dateEnd = dayjs(dates.end).format(DATE_FORMATS.CHOSED_DATE);
